...
# 获取LSTM数据集长度
print("LSTM_train_dataset_size:", len(LSTM_train_dataset))
print("LSTM_val_dataset_size:", len(LSTM_val_dataset))
print("LSTM_test_dataset_size:", len(LSTM_test_dataset))
...
# 获取数据集样本数
print("GCN_train_data_size:", len(GCN_train_dataset))
print("GCN_val_data_size:", len(GCN_val_dataset))
print("GCN_test_data_size:", len(GCN_test_dataset))
...
output = model2(gcn_x, edge_index, batch)
...
if __name__ == '__main__':
    ...
    print(f"============ 模型测试开始 ============")
    # 模型测试
    test_model = torch.load("./all_data/model_save/Bytecode+CFG.pth")
    # 模型验证(评估当前训练模型的性能)
    test_model.eval()
