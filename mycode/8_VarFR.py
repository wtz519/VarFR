...
if __name__ == '__main__':
    ...
    # 模型测试
    test_model = torch.load("./all_data/model_save/fusion_model_CNN.pth")
    # 模型验证(评估当前训练模型的性能)
    test_model.eval()
    ...
    with torch.no_grad():
        for Bytecode_data, Var_data, CFG_data in zip(Bytecode_test_loader, Var_test_loader, CFG_test_loader):
            ...
            # 获取 Bytecode 数据
            Bytecode_inputs, targets, targets_mask, train_csv_names = Bytecode_data
            Bytecode_inputs, targets, targets_mask = Bytecode_inputs.to(device), targets.to(device), targets_mask.to(
                device)
            csv_name = train_csv_names[0].split(".txt")[0] + ".csv"
