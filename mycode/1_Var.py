...
# 实例化数据集对象
train_dataset = VarDataset("./all_data/split_data/train_pkl.pkl")
val_dataset = VarDataset("./all_data/split_data/val_pkl.pkl")
test_dataset = VarDataset("./all_data/split_data/test_pkl.pkl")
...
# 创建模型实例
model = SentenceConv1D(input_dim=768, sentence_length=20, out_channels=128, kernel_size=3)
...
if __name__ == '__main__':
    ...
    # 模型测试
    print(f"============ 模型测试开始 ============")
    test_model = torch.load("./all_data/model_save/Var_model.pth")
    # 模型验证(评估当前训练模型的性能)
    test_model.eval()
